...
from general import InvalidEntityCategoryException
...
from polygon import sample_from_polygon, transform_polygon
...
class World:
    ...
    def set_inflation_radius(self, inflation_radius=0.0, ignore_radius=False):
        """
        ...
        for entity in itertools.chain(self.rooms, self.hallways):
            if ignore_radius and entity.name == 'hall_kitchen_bathroom':
                continue
    ...
    def add_robot(self, robot, loc=None, pose=None, ignore_radius=False):
        """
        ...
        if new_inflation_radius > old_inflation_radius:
            self.set_inflation_radius(new_inflation_radius, ignore_radius)
    ...
    def get_object_spawn_by_name(self, name):
        spawns = self.get_object_spawns()
        for spawn in spawns:
            if spawn.name == name:
                return spawn
        return None
