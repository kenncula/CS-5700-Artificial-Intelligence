...
def sentence3() -> Expr:
    ...
    one = PacmanAlive_1 % disjoin(PacmanAlive_0 & ~PacmanKilled_0, ~PacmanAlive_0 & PacmanBorn_0)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    if successorAxioms is not None and t >= 1:
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(pacphysicsAxioms(1,all_coords,non_outer_wall_coords,walls_grid, None, None))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne(possible_squares))

        goal_model = findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(KB))
        if goal_model:
            return extractActionSequence(goal_model,actions)
        
        possible_actions = []
        for a in actions:
            possible_actions.append(PropSymbolExpr(a, time=t))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        check_food_eated = []
        for x,y in food:
            check_food_eated.append(~PropSymbolExpr(food_str, x, y, time=t))

        goal_model = findModel(conjoin(check_food_eated) & conjoin(KB))
        ...
        for x,y in food:
            pacman_at_xyt1 = PropSymbolExpr(pacman_str,x,y,time=t)
            food_at_xyt1 = PropSymbolExpr(food_str,x,y,time=t)
            food_at_xyt2 = PropSymbolExpr(food_str,x,y,time=t+1)
            KB.append(~food_at_xyt1 | ((pacman_at_xyt1 & food_at_xyt1) % ~food_at_xyt2))
    "*** END YOUR CODE HERE ***"

#______________________________________________________________________________
# QUESTION 6

def pacphysicsandstuff(KB,t,all_coords,non_outer_wall_coords,agent,grid,sensorAxioms,successorAxiom, perceptRules):
    KB.append(pacphysicsAxioms(t,all_coords,non_outer_wall_coords,grid,sensorAxioms,successorAxiom))
    KB.append(PropSymbolExpr(agent.actions[t],time=t))
    KB.append(perceptRules(t, agent.getPercepts()))


def possibleLocationsProver(KB,t,x,y,possible_locations):
    pacPos =  PropSymbolExpr(pacman_str,x,y,time=t)
    pacman_at_xyt = entails(conjoin(KB), pacPos)
    pacman_not_at_xyt = entails(conjoin(KB), ~pacPos)

    if pacman_at_xyt:
        KB.append(pacPos)
        
    elif pacman_not_at_xyt:
        KB.append(~pacPos)
    if not pacman_not_at_xyt:
        possible_locations.append((x,y))

def possibleWallsProver(KB,x,y,map):
    wallPos =  PropSymbolExpr(wall_str,x,y)
    wall_at_xyt = entails(conjoin(KB), wallPos)
    wall_not_at_xyt = entails(conjoin(KB), ~wallPos)
    
    if wall_at_xyt:
        KB.append(wallPos)
        map[x][y] = 1 
    elif wall_not_at_xyt:
        KB.append(~wallPos)
        map[x][y] = 0
    else:
        map[x][y] = -1
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = [ ]
        "*** BEGIN YOUR CODE HERE ***"
        pacphysicsandstuff(KB,t,all_coords,non_outer_wall_coords,agent,walls_grid,sensorAxioms,allLegalSuccessorAxioms, fourBitPerceptRules)
        
        for x,y in non_outer_wall_coords:
            possibleLocationsProver(KB,t,x,y,possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        "*** BEGIN YOUR CODE HERE ***"
        pacphysicsandstuff(KB,t,all_coords,non_outer_wall_coords,agent,known_map,sensorAxioms,allLegalSuccessorAxioms,fourBitPerceptRules)

        for x,y in non_outer_wall_coords:
            possibleWallsProver(KB,x,y,known_map)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        "*** BEGIN YOUR CODE HERE ***"
        pacphysicsandstuff(KB,t,all_coords,non_outer_wall_coords,agent,known_map,SLAMSensorAxioms,SLAMSuccessorAxioms,numAdjWallsPerceptRules)
