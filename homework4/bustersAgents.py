...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        numGhosts = sum(bool(x) for x in livingGhosts)
        shortest = 9999999
        ...
        for action in legal:
            ...
            for ghost in range(numGhosts):
                mostLikelyGhostPos = livingGhostPositionDistributions[ghost].argMax()
                mazeDistance = self.distancer.getDistance(successorPosition,mostLikelyGhostPos)
                if mazeDistance < shortest:
                    shortest = mazeDistance
                    result = action
